...
- `PGHOST`, `PGPORT`, `PGUSER`, `PGPASSWORD`, `PGDATABASE`: Credenciales de PostgreSQL
- `SESSION_SECRET`: Secreto para sesiones (debe generarse uno aleatorio)
- `NODE_ENV=development`: Modo desarrollo

### Configuración del archivo .env

1. **Copiar plantilla**: `copy .env.windows .env`
2. **Editar archivo**: `notepad .env`
3. **Reemplazar** `TU_CONTRASEÑA` con tu contraseña de PostgreSQL
4. **Generar SESSION_SECRET**:
   ```powershell
   node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
   ```
5. **Guardar** el archivo `.env`

**Archivos de configuración:**
- `.env.windows` → Plantilla para copiar a `.env`
- `INSTALACION_WINDOWS.md` → Guía completa de instalación paso a paso
...
- [x] Configuración lista para Windows 11 (puerto 3000)
- [x] Scripts de instalación para Windows
- [x] Documentación completa de instalación en Windows
